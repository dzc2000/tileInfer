...
def fused_postproj_recurrent_kernel_v2(B, num_k_heads, num_v_heads, Dk, Dv, BLOCK_DK, FUSE_NORM, dtype):
    ...
    def kernel(
        ...
        with T.Kernel(num_v_heads, B, threads=THREADS) as (h, bid):
            ...
            S_tile = T.alloc_shared((BLOCK_DK, Dv), dtype)
            S_decayed_tile = T.alloc_shared((BLOCK_DK, Dv), T.float32)
            ...
            for r_start in T.Pipelined(num_tiles, num_stages=2):
                ...
                for j in T.Parallel(Dv):
                    ...
                    for i in T.serial(BLOCK_DK):
                        s_val = S_decayed_tile[i, j]
                        dot_k[0] = dot_k[0] + s_val * T.cast(k_tile[i], T.float32)
                        dot_q[0] = dot_q[0] + s_val * T.cast(q_tile[i], T.float32)
                    accum_sk[j] = accum_sk[j] + dot_k[0]
                    accum_sq[j] = accum_sq[j] + dot_q[0]
            ...
            qk_dot = T.alloc_shared((1,), T.float32)
            ...
            for j in T.Parallel(Dv):
                output[j] = accum_sq[j] + qk_dot[0] * delta[j]

            S_tile2 = T.alloc_shared((BLOCK_DK, Dv), dtype)
            for r_start in T.Pipelined(num_tiles, num_stages=2):
                T.copy(State[bid, h, r_start * BLOCK_DK, 0], S_tile2)
                T.copy(k_full[r_start * BLOCK_DK], k_tile)

                for i, j in T.Parallel(BLOCK_DK, Dv):
                    new_val = T.cast(S_tile2[i, j], T.float32) * decay + T.cast(k_tile[i], T.float32) * delta[j]
                    NewState[bid, h, r_start * BLOCK_DK + i, j] = T.cast(new_val, dtype)
