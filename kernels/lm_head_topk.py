...
def fused_lm_head_argmax(x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    if x.dim() == 1:
        ...
        # 编译并执行 1D Argmax 归约
        kernel2 = argmax_reduce_kernel(num_blocks_n, block_size=256, dtype=dtype)
        ...
        # GEMM Tiling 参数 (经调参确定，见 tune_gemm)
        block_M = 64
        block_N = 128
        block_K = 64
        num_stages = 3
        ...
        # block_m_reduce=4：每 Block 串行 allreduce 从 16 次降到 8 次 (max+min_idx/行)，
        # Grid 从 8 升到 16 块仍 < SM 数 (单 wave)，靠减少串行同步降低 kernel2 延迟。
        block_m_reduce = 4
        threads_reduce = 128
