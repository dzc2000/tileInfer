...
A high-performance inference engine for **Qwen3.6-27B**, built on custom TileLang kernels, FlashQLA linear attention, and vLLM-style serving features.
...
| **FlashQLA** | `chunk_gated_delta_rule` from [QwenLM/FlashQLA](https://github.com/QwenLM/FlashQLA) for efficient DeltaNet linear attention prefill |
| **Paged KV Cache** | Block-based KV cache with prefix caching, inspired by vLLM |
...
| **Prefix Cache** | Reuse KV blocks for shared prompt prefixes |
...
[Embed + RoPE]
  |
  v
[Layer 0-47: DeltaNet]  --> FlashQLA chunk_gated_delta_rule (prefill)
  |                          fused causal_conv1d_update (decode)
  v
[Layer 48-63: GQA Attention] --> TileLang flashattn varlen (prefill)
  |                               TileLang GQA decode with KV cache
  v
[Norm + LM Head]
  |
  v
...
# Install FlashQLA (for DeltaNet linear attention)
...
│   ├── deltanet_fused.py   # Fused DeltaNet post-projection
...
# Multi-GPU TP scaling (run each TP size separately)
python tests/bench_multi_gpu.py --model /path/to/Qwen3.6-27B --tp-size 1
python tests/bench_multi_gpu.py --model /path/to/Qwen3.6-27B --tp-size 2
python tests/bench_multi_gpu.py --model /path/to/Qwen3.6-27B --tp-size 4
...
6. **Fused DeltaNet Post-projection** — 17 ops -> 1 kernel per layer
7. **FlashQLA** — Efficient chunk-based linear attention for DeltaNet prefill
...
- transformers >= 4.40.0
- tilelang >= 0.1.0
- FlashQLA (install manually from source)
