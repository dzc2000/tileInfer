...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const bfloat16_t* __restrict__ K_cache, bfloat16_t* __restrict__ Output, const bfloat16_t* __restrict__ Q, const bfloat16_t* __restrict__ V_cache, const int* __restrict__ block_table, const int* __restrict__ seqlen_kv) {
  ...
  int seqlen_kv_b = seqlen_kv[0];
  #pragma unroll
  for (int i = 0; i < 3; ++i) {
    *(uint1*)(((bfloat16_t*)Q_shared) + (((((((((((int)threadIdx.x) & 127) >> 5) * 1024) + (i * 128)) + ((((int)threadIdx.x) >> 7) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + (i >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + (i & 1)) & 1) * 16)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(Q + (((((int)blockIdx.y) * 1536) + (i * 512)) + (((int)threadIdx.x) * 2)));
  }
  #pragma unroll
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
  ...
  for (int k = 0; k < (((seqlen_kv_b + 63) >> 6) - 1); ++k) {
    ...
    *(float4*)(acc_s + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int sub_2 = 0; sub_2 < 4; ++sub_2) {
      int condval;
      if ((((k * 2) + (sub_2 >> 1)) < 6241)) {
        condval = block_table[(((k * 4) + sub_2) + 4)];
      } else {
        condval = 0;
      }
    ...
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    for (int i_9 = 0; i_9 < 2; ++i_9) {
      ...
      scores_sum[i_9] = tl::AllReduce<tl::SumOp, 256, 32, 0>::run(scores_sum[i_9], (&(((float*)workspace)[0])));
    ...
    for (int sub_3 = 0; sub_3 < 4; ++sub_3) {
      int condval_2;
      if ((((k * 2) + (sub_3 >> 1)) < 6241)) {
        condval_2 = block_table[(((k * 4) + sub_3) + 4)];
      } else {
        condval_2 = 0;
      }
  ...
  if (1 <= seqlen_kv_b) {
    ...
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    for (int i_16 = 0; i_16 < 2; ++i_16) {
      ...
      scores_max_clear_1[i_16] = tl::AllReduce<tl::MaxOp, 256, 32, 0>::run(scores_max_clear_1[i_16], (&(((float*)workspace_2)[0])));
    ...
    for (int i_19 = 0; i_19 < 2; ++i_19) {
      ...
      scores_sum[i_19] = tl::AllReduce<tl::SumOp, 256, 32, 0>::run(scores_sum[i_19], (&(((float*)workspace_1)[0])));
  ...
  for (int i_26 = 0; i_26 < 3; ++i_26) {
    *(uint1*)(Output + (((((int)blockIdx.y) * 1536) + (i_26 * 512)) + (((int)threadIdx.x) * 2))) = *(uint1*)(((bfloat16_t*)O_shared) + ((i_26 * 512) + (((int)threadIdx.x) * 2)));
  }
}
