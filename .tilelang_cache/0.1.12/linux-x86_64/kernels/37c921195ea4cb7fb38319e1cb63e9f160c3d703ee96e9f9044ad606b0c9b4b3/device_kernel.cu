...
extern "C" __global__ void __launch_bounds__(256, 1) kernel_kernel(const float* __restrict__ Beta, float* __restrict__ FinalState, const float* __restrict__ G, const float* __restrict__ InitState, const bfloat16_t* __restrict__ K, bfloat16_t* __restrict__ Output, const bfloat16_t* __restrict__ Q, const bfloat16_t* __restrict__ V) {
  ...
  void* S = ((void*)((char*)buf_dyn_shmem + 0));
  void* O_buf = ((void*)((char*)buf_dyn_shmem + 65536));
  void* K_buf = ((void*)((char*)buf_dyn_shmem + 81920));
  void* Q_buf = ((void*)((char*)buf_dyn_shmem + 90112));
  void* V_buf = ((void*)((char*)buf_dyn_shmem + 98304));
  void* A = ((void*)((char*)buf_dyn_shmem + 106496));
  void* beta_buf = ((void*)((char*)buf_dyn_shmem + 110592));
  void* cum_g = ((void*)((char*)buf_dyn_shmem + 110720));
  float acc[1];
  float acc_1[1];
  float acc_2[1];
  ...
  for (int chunk_idx = 0; chunk_idx < 7; ++chunk_idx) {
    __syncthreads();
    #pragma unroll
    for (int i_1 = 0; i_1 < 2; ++i_1) {
      *(uint4*)(((bfloat16_t*)Q_buf) + ((i_1 * 2048) + (((int)threadIdx.x) * 8))) = *(uint4*)(Q + (((((chunk_idx * 32768) + (i_1 * 16384)) + ((((int)threadIdx.x) >> 4) * 1024)) + ((((int)blockIdx.x) / 3) * 128)) + ((((int)threadIdx.x) & 15) * 8)));
      *(uint4*)(((bfloat16_t*)K_buf) + ((i_1 * 2048) + (((int)threadIdx.x) * 8))) = *(uint4*)(K + (((((chunk_idx * 32768) + (i_1 * 16384)) + ((((int)threadIdx.x) >> 4) * 1024)) + ((((int)blockIdx.x) / 3) * 128)) + ((((int)threadIdx.x) & 15) * 8)));
    }
    #pragma unroll
    for (int i_2 = 0; i_2 < 2; ++i_2) {
      *(uint4*)(((bfloat16_t*)V_buf) + ((i_2 * 2048) + (((int)threadIdx.x) * 8))) = *(uint4*)(V + (((((chunk_idx * 98304) + (i_2 * 49152)) + ((((int)threadIdx.x) >> 4) * 3072)) + (((int)blockIdx.x) * 128)) + ((((int)threadIdx.x) & 15) * 8)));
    }
    if (((int)threadIdx.x) < 32) {
      ((float*)beta_buf)[((int)threadIdx.x)] = Beta[(((chunk_idx * 768) + (((int)threadIdx.x) * 24)) + ((int)blockIdx.x))];
    }
    ((float*)cum_g)[0] = G[((chunk_idx * 768) + ((int)blockIdx.x))];
    __syncthreads();
    for (int i_3 = 0; i_3 < 31; ++i_3) {
      ((float*)cum_g)[(i_3 + 1)] = (((float*)cum_g)[i_3] + G[((((chunk_idx * 768) + (i_3 * 24)) + ((int)blockIdx.x)) + 24)]);
    }
    __syncthreads();
    #pragma unroll
    ...
    for (int i_7 = 0; i_7 < 4; ++i_7) {
      ...
      *(uint2*)(Output + (((((chunk_idx * 98304) + (i_7 * 24576)) + ((((int)threadIdx.x) >> 5) * 3072)) + (((int)blockIdx.x) * 128)) + ((((int)threadIdx.x) & 31) * 4))) = *(uint2*)(Output_local_cast + 0);
    }
