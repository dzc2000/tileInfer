...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(bfloat16_t* __restrict__ S, const bfloat16_t* __restrict__ W, const bfloat16_t* __restrict__ X, bfloat16_t* __restrict__ Y) {
  ...
  float w_local[1];
  x_local[0] = ((float)X[((((int)blockIdx.y) * 128) + ((int)threadIdx.x))]);
  acc[0] = 0x0p+0f/*0.000000e+00*/;
  for (int i = 0; i < 4; ++i) {
    if (i < 3) {
      s_local[0] = S[((((((int)blockIdx.y) * 512) + (((int)threadIdx.x) * 4)) + i) + 1)];
    } else {
      s_local[0] = ((bfloat16_t)x_local[0]);
    }
    S[(((((int)blockIdx.y) * 512) + (((int)threadIdx.x) * 4)) + i)] = s_local[0];
  ...
  Y[((((int)blockIdx.y) * 128) + ((int)threadIdx.x))] = ((bfloat16_t)acc[0]);
}
