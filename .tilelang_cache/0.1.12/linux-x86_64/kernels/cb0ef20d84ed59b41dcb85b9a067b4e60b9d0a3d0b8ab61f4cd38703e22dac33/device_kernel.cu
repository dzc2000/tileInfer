...
extern "C" __global__ void __launch_bounds__(64, 1) main_kernel(const bfloat16_t* __restrict__ A, const bfloat16_t* __restrict__ B, bfloat16_t* __restrict__ C) {
  float C_accum[1];
  bfloat16_t A_local[8];
  bfloat16_t B_local[8];
  float C_reduced[1];
  C_accum[0] = 0x0p+0f/*0.000000e+00*/;
  for (int bk = 0; bk < 20; ++bk) {
    *(uint4*)(A_local + 0) = *(uint4*)(A + ((bk * 256) + (((int)threadIdx.x) * 8)));
    *(uint4*)(B_local + 0) = *(uint4*)(B + ((((((int)blockIdx.x) * 10240) + (((int)threadIdx.y) * 5120)) + (bk * 256)) + (((int)threadIdx.x) * 8)));
    for (int k = 0; k < 8; ++k) {
  ...
  C[((((int)blockIdx.x) * 2) + ((int)threadIdx.y))] = ((bfloat16_t)red_buf0[0]);
}
