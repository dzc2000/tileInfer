...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(bfloat16_t* __restrict__ K_Cache, const bfloat16_t* __restrict__ Key, const int* __restrict__ Slot_Mapping, bfloat16_t* __restrict__ V_Cache, const bfloat16_t* __restrict__ Value) {
  ...
  void* slot_shared = ((void*)((char*)buf_dyn_shmem + 32768));
  #pragma unroll
  for (int i = 0; i < 8; ++i) {
    bfloat16_t broadcast_var = bfloat16_t(0x0p+0f/*0.000000e+00*/);
    uint4 condval;
    if ((((i * 2) + (((int)threadIdx.x) >> 6)) < 1)) {
      condval = *(uint4*)(Key + ((i * 1024) + (((int)threadIdx.x) * 8)));
    } else {
  ...
  for (int i_1 = 0; i_1 < 8; ++i_1) {
    ...
    uint4 condval_1;
    if ((((i_1 * 2) + (((int)threadIdx.x) >> 6)) < 1)) {
      condval_1 = *(uint4*)(Value + ((i_1 * 1024) + (((int)threadIdx.x) * 8)));
    } else {
  ...
  if (((int)threadIdx.x) < 16) {
    ...
    if ((((int)threadIdx.x) < 1)) {
      condval_2 = Slot_Mapping[((int)threadIdx.x)];
    } else {
      condval_2 = 0;
    }
