"""
Manual decode loop with CUDA graph capture for Qwen3.6-27B.
...
Pattern:
  ...
  2. Warmup: 5 eager decode steps (warms up CUDA kernels)
...
from kernels.lm_head_topk import fused_lm_head_argmax
...
def generate_eager(
    ...
    Uses CUDA event timing. Single GPU only.
...
def generate_cuda_graph(
    ...
    # --- Warmup decode (eager, warms up CUDA kernels) ---
    ...
    def _decode_step():
        ...
        # Fused lm_head + argmax via TileLang: avoids 248K logit materialization
        # TileLang kernel allocations are captured by CUDA graph — acceptable for
        # benchmark use (graph lives for ~256 replays then freed).
        h_flat = h.squeeze()  # [5120]
        return fused_lm_head_argmax(h_flat, model.lm_head.weight).view(1, 1)
...
def benchmark_decode(
    ...
    gen_kwargs = {"model": model, "input_ids": input_ids, "max_new_tokens": max_new_tokens,
                  "cache": cache, "device": device}

    # Warmup runs
    print(f"Warmup: {warmup_runs} runs...")
    for i in range(warmup_runs):
        _, timing = gen_fn(**gen_kwargs)
    ...
    results = {
        ...
        "decode_tok_s": {
            ...
            "p95": round(tok_s_sorted[min(int(len(tok_s_sorted) * 0.95), len(tok_s_sorted) - 1)], 1),
            "p99": round(tok_s_sorted[min(int(len(tok_s_sorted) * 0.99), len(tok_s_sorted) - 1)], 1),
    ...
    def _decode_step():
        ...
        for layer in model.model.layers:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                x_normed_next = None
        h = model.model.norm(h)
        # Fused batched lm_head + argmax via TileLang: avoids 248K logit materialization
        # TileLang kernel allocations are captured by CUDA graph — acceptable for
        # benchmark use (graph lives for ~256 replays then freed).
        h_flat = h.squeeze(1)  # [B, 5120]
        return fused_lm_head_argmax(h_flat, model.lm_head.weight).view(batch_size, 1)
